...
| `/kickout manual`    | Manage server. | None                | `hour`, `channel_name`, `server_id`             |

When `channel_name` is omitted, the manual check-in, room-order, and kickout commands use the current Discord channel to resolve the running room. The optional `template` on `/checkin manual` is an advanced Handlebars message template. The optional `heal` value tells room order how many healer positions are needed.

`/kickout manual` is useful only when the running channel has a configured room role. TiaraBot rejects the command from minute 40 through minute 59 so fillers scheduled through minute 00 of the next hour are not removed early.
...
All `/channel` commands require Manage Server or Administrator in the target server.

| Command                | Required parameters | Optional parameters                                                  |
| ---------------------- | ------------------- | -------------------------------------------------------------------- |
| `/channel list_config` | None                | `channel`, `server_id`                                               |
| `/channel set`         | None                | `channel`, `running`, `name`, `role`, `checkin_channel`, `server_id` |
| `/channel unset`       | None                | `channel`, `running`, `name`, `role`, `checkin_channel`, `server_id` |

`channel` defaults to the current Discord channel. On `/channel unset`, set each field flag to true when that stored value should be cleared. The optional `role` is the running-room visibility role. If it is not set, TiaraBot does not grant or remove room access roles and `/kickout manual` is not needed.

These commands store TiaraBot's channel mapping. They do not change Discord channel permission overwrites. If a room role is configured, TiaraBot needs Manage Roles and its bot role must sit above the managed room role.

## Message controls

| Control                          | Caller access       | Parameters | Notes                                                                                           |
| -------------------------------- | ------------------- | ---------- | ----------------------------------------------------------------------------------------------- |
| Check-in button                  | Check-in filler.    | None       | Records that filler and restores the configured room role when the room uses one.               |
