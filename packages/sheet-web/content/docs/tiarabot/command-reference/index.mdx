...
| `/fillers list`  | Monitor.                                                    | `channel_name`, `start_hour`, `finish_hour` | `server_id`                 |
...
`/fillers list` returns each unique filler scheduled in the selected running channel from
`start_hour` through `finish_hour`, inclusive. TiaraBot reads Fill cells, skips blanks and break
rows, includes rows that are not yet publicly visible, and collapses repeated assignments for the same linked
account. The result is a fenced code block. Linked accounts appear as raw Discord user mention
tokens such as `<@123456789012345678>`. Copy the lines from the code block and paste them into a
Discord message to mention those fillers. The command does not ping anyone in its own response.

When a sheet name maps to multiple accounts, TiaraBot cannot identify the account safely and shows
the sheet name instead. If the list is too large for the response or an unresolved name contains a
code fence, TiaraBot attaches the complete list as `fillers.txt`, with one mention token or
unresolved name per line. The command does not include Overfill or Standby entries and does not
change the schedule.
...
The same fields are available from a server's **Dashboard → Settings → Server** page with the
same permission requirement, including the optional **Update announcement channel** setting.
Configure the field there or with the slash commands below.
...
| `/server set announcement_channel`   | `channel`           | `server_id`         |
| `/server unset monitor_channel`      | None                | `server_id`         |
| `/server unset announcement_channel` | None                | `server_id`         |
...
`announcement_channel` is the optional destination for TiaraBot product and service updates. Set it
to a text or announcement channel where TiaraBot can view and send messages. Unsetting it preserves
the default update-announcement routing: the Discord system channel, then `#general`, then the
first sendable text or announcement channel. If the configured channel is unavailable or cannot
accept a message, TiaraBot does not silently use a different channel.

The `update-announcements` flag remains the delivery gate. The flag must be enabled for updates to
be sent, whether or not `announcement_channel` is configured. Channel configuration does not opt a
server in or out of the feed.
