...
const asyncResultError = <A, E>(result: AsyncResult.AsyncResult<A, E>) =>
  AsyncResult.match(result, {
    onInitial: () => undefined,
    onFailure: ({ cause }) => messageFromUnknown(Cause.squash(cause)),
    onSuccess: () => undefined,
  });
...
const clientsValue = (result: ReturnType<typeof useSupportedNotificationClientsResult>) => {
  return AsyncResult.match(result, {
    onInitial: () => [] as readonly SupportedNotificationClient[],
    onFailure: (failure) =>
      Option.match(failure.previousSuccess, {
...
const preferencesDisabledFor = (
  data: ReturnType<typeof usePreferenceData>,
  saveState: ReturnType<typeof useSavePreference>,
) => [saveState.saving, data.waiting, data.loadError !== undefined].some(Boolean);

const displayNameFor = (user: { readonly global_name: string | null; readonly username: string }) =>
...
function ErrorBanner(props: { readonly error: string | undefined }) {
  ...
  return (
    <div
      role="alert"
      className="border-b border-red-400/30 bg-red-950/30 px-6 py-3 text-sm text-red-100"
    >
      {props.error}
    </div>
  );
...
function useInitializedForm(props: {
  ...
  useEffect(() => {
    if (!formInitialized && !props.waiting && props.loadError === undefined) {
      setForm(initialForm(props.config, props.discordClients));
      setFormInitialized(true);
    }
  }, [formInitialized, props.config, props.discordClients, props.loadError, props.waiting]);
...
export function NotificationPreferencesPage() {
  ...
  const disabled = preferencesDisabledFor(data, saveState);
  const displayName = displayNameFor(data.currentUser);
