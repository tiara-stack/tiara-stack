...
const makeAddSubCommand = CommandHelper.makeSubCommand(
  (builder) =>
    builder
      .setName("add")
  ...
  Effect.fn("workspace.add")(function* (command) {
    ...
    yield* response.reply({
      content: action === "updated" ? `Workspace "${name}" updated!` : `Workspace "${name}" added!`,
    });
  }),
);

const makeRemoveSubCommand = CommandHelper.makeSubCommand(
  (builder) =>
    builder
      .setName("remove")
  ...
  Effect.fn("workspace.remove")(function* (command) {
    ...
    const name = command.optionValue("name");
    const workspace = yield* Effect.tryPromise(() => getWorkspaceByUserAndName(userId, name)).pipe(
      Effect.catch((error) =>
        Effect.gen(function* () {
          const message = error instanceof Error ? error.message : "Unknown error";
          yield* response.reply({
