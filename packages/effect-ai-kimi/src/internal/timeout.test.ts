...
describe("runWithAbortTimeout", () => {
  it("returns a run result that completes before the timeout", async () => {
    await expect(
      runWithAbortTimeout({
        ...
        abort: () => undefined,
        timeoutMs: 100,
        timeoutError: () => new KimiTimeout({ timeoutMs: 100 }),
      }),
    ).resolves.toBe("ok");
  });
  ...
  it("aborts at the deadline and reports timeout after the run settles", async () => {
    ...
    const result = runWithAbortTimeout({
      ...
      timeoutMs: 1,
      cleanupGraceMs: 100,
      timeoutError: () => new KimiTimeout({ timeoutMs: 1 }),
    });
    ...
    await expect(result).rejects.toBeInstanceOf(KimiTimeout);
  });
  ...
  it("treats zero as an immediate timeout", async () => {
    ...
    const result = runWithAbortTimeout({
      ...
      timeoutMs: 0,
      cleanupGraceMs: 1,
      timeoutError: () => new KimiTimeout({ timeoutMs: 0 }),
    });
