...
const existingUser = {
  ...
  scopes: new Set(["workflow.dispatch" as SheetAuthOAuthScope]),
...
describe("SheetAuthTokenAuthorizationLive", () => {
  it.effect("reuses an existing SheetAuthUser before rejecting bearer fallback", () =>
    Effect.gen(function* () {
      ...
      if (Option.isSome(user)) {
        ...
        expect(user.value.scopes.has("workflow.dispatch")).toBe(true);
  ...
  it.effect("rejects a forwarded bearer when no existing user is available", () =>
    Effect.gen(function* () {
  ...
  it.effect("round-trips forwarded workflow users through the bearer envelope", () =>
    Effect.gen(function* () {
      ...
      expect(user.scopes.has("workflow.dispatch")).toBe(true);
