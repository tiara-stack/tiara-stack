...
describe("SheetAuthTokenAuthorizationLive", () => {
  it("allows workflow dispatch calls with workflow.dispatch scope", async () => {
  ...
  it("rejects workflow dispatch calls without workflow.dispatch scope", async () => {
    ...
    await expect(effect).rejects.toThrow("dispatch.checkin requires workflow.dispatch scope");
  });

  it("rejects protected workflow RPCs without a declared scope policy", async () => {
