...
const runReadiness = (
  ...
  options: {
    ...
    readonly parameterSets?: unknown[][];
  ...
  effect.pipe(
    Effect.provideService(
      SqlClient.SqlClient,
      fakeSql(rows, options.queries, options.parameterSets),
    ),
...
describe("cluster readiness", () => {
  it.effect("marks the current runner ready when its healthy heartbeat is recent", () =>
    Effect.gen(function* () {
      ...
      expect(parameterSets.flat()).toContain(600);
    }),
  );
  ...
  it.effect("marks the runner fleet ready when any healthy runner has a recent heartbeat", () =>
    Effect.gen(function* () {
      ...
      expect(query).toContain(") = ?");
      const lockCountSubquery = query.split("SELECT COUNT(*)")[1]?.split(") = ?")[0];
      expect(lockCountSubquery).toContain('"sheet_workflows_locks".address = runner.address');
    }),
  );
  ...
  it.effect("uses current runner readiness for browser-runner role", () =>
    Effect.gen(function* () {
      const queries: string[] = [];
      const parameterSets: unknown[][] = [];
      const ready = yield* runReadiness(isWorkflowApiReady, [{ ready: true }], {
        env: { SHEET_WORKFLOWS_ROLE: "browser-runner" },
        parameterSets,
        queries,
      });
      ...
      expect(ready).toBe(true);
      expect(queries.join("\n")).toContain('"sheet_workflows_runners".address = ?');
      expect(parameterSets.flat()).toContain(300);
      expect(parameterSets.flat()).not.toContain(600);
    }),
  );
});
