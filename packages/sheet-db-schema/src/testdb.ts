...
const sqlType = (column: ColumnSnapshot): string => {
  const varcharLength = column.config?.length;
  const varcharType = Predicate.isNumber(varcharLength) ? `varchar(${varcharLength})` : "varchar";
      ...
      Effect.tryPromise({
        try: () =>
          zqlDb.transaction(async (tx) => {
            ...
            for (const table of canonicalSeedTableNames) {
              const rows = seeds[table];
              if (!rows) continue;
