...
add column "monitor_dm_enabled" boolean default false;
`).withoutTransform;

  // --> statement-breakpoint
  yield* sql.unsafe(`
update "sheet_db_config_user_platform"
set "monitor_dm_enabled" = false
where "monitor_dm_enabled" is null;
`).withoutTransform;

  // --> statement-breakpoint
  yield* sql.unsafe(`
alter table "sheet_db_config_user_platform"
alter column "monitor_dm_enabled" set not null;
`).withoutTransform;

  // --> statement-breakpoint
  yield* sql.unsafe(`
CREATE INDEX "sheet_db_config_user_platform_monitor_dm_recipient_idx"
ON "sheet_db_config_user_platform" ("platform", "user_id")
WHERE "monitor_dm_enabled" = true
