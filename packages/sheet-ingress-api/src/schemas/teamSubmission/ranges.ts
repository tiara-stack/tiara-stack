import type { ParsedOshi, ParsedTeamEntry } from "./teamSubmission";
...
export const parseA1Start = (range: string, options: A1RangeOptions = {}): A1Cell | null => {
  const match = a1RangeRegex.exec(range.trim());
  const groups = match?.groups;
  const sheet = groups?.quotedSheet?.replaceAll("''", "'") ?? groups?.sheet?.trim();
  if (!sheet || !groups?.column || (!groups.row && !options.allowMissingRow)) {
    return null;
  }
...
export const cellForRow = (range: string, row: number, options: A1RangeOptions = {}) => {
  const start = parseA1Start(range, options);
...
export const appendRangeForCells = (
  ...
  const cells = [
    ["playerColumn", parseA1Start(playerNameRange, options)],
    ["teamColumn", parseA1Start(teamNameRange, options)],
    ...(oshiRange === null ? [] : ([["oshiColumn", parseA1Start(oshiRange, options)]] as const)),
  ] as const;
...
export const appendedRowTarget = (
  { rowIndex, playerNameRange, teamNameRange, oshiRange }: TeamSubmissionRowTarget,
  options: A1RangeOptions = {},
